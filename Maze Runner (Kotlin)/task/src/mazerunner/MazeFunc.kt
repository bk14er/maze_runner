...
class Maze(private var size: Int = 0, image: String = "") {
    ...
    private fun decodeCell(img: String, r: Int, c: Int) { if (isPass(img, r, c)) maze[r][c] = Cell.PASS }
    ...
    private fun doExit(crd: Crd, from: Crd) = findExit(crd, from).let { it?.apply { setCell(from, Cell.SOLVE) } }
